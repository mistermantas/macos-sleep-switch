...
| `RemoteArtifactOffer` | `sourceDeviceID` (String), `state` (String), `createdAt` (Date), `expiresAt` (Date), `payload` (Bytes), `asset` (Asset) |

Create query indexes for `MacStatus.deviceID`, `MacStatus.expiresAt`, `InsightsHistory.deviceID`, `InsightsHistory.expiresAt`, `RemoteCommand.targetDeviceID`, `RemoteCommand.state`, `RemoteContextTransfer.targetDeviceID`, `RemoteContextTransfer.state`, `RemoteArtifactOffer.sourceDeviceID`, and `RemoteArtifactOffer.state`. Deploy the development schema, then promote the same schema to **Production** before archiving. Release builds use the production CloudKit environment; Debug builds continue to use Development. The app only uses the private database; there is no public record exposure.

`RemoteContextTransfer` is an explicit iPhone/iPad-to-Mac handoff, not a file browser. A user picks one item (maximum 25 MB); it expires after 24 hours. The optional **Send to Sleep Switch** Share Sheet extension only creates a private, short-lived App Group receipt. The user must open the companion and explicitly confirm the selected Mac before any CloudKit upload occurs. The Mac validates the delivered item, copies it to its private Remote Inbox, clears the CloudKit asset, and never places it in a repository or sends it to an agent automatically. A later **Place…** action on the Mac requires the owner to select a destination folder and creates a non-overwriting local copy.

`RemoteArtifactOffer` is the reverse, explicit Mac-to-iPhone/iPad handoff. A Mac user chooses one result file (maximum 25 MB); the companion lists only the offered name, size, and age until the user taps **Get**. It expires after 24 hours. The companion copies the selected asset into private app storage before exposing Quick Look or the system share sheet. It does not expose a Mac path, directory listing, project contents, or any automatic artifact download.

...
The optional **Prevent Sleep Even With Lid Closed** mode in the GitHub download uses a signed power helper. Choose **Awake Mode → Set Up Lid-Closed Mode…** and approve Sleep Switch once in **System Settings → General → Login Items & Extensions**. Later sessions start and stop without administrator scripts or password prompts. The helper accepts only sleep-control requests from the signed app, restores normal sleep on disconnect or lease expiry, and keeps a root-owned recovery record for crash recovery. It preserves sleep overrides that were already active before its session. The battery floor and external-power requirement still apply. The separate cooling helper is unaffected.
...
Manual-session completion returns the resulting Mac state in the command acknowledgement. The Start/Stop control updates immediately, and delayed status queries cannot undo a confirmed result. Manual sessions have a live countdown in the app. Home Screen and Lock Screen widgets show the selected Mac’s battery, temperature, and active agent sessions. Optional heat alerts use conservative defaults: 85°C while a non-system cooling profile is active, with separate 30- and 60-minute milestones. Alert permission and every threshold are controlled in the iPhone settings screen.

**Live Activities:** Open Live Activity on the dashboard or Settings → Live Activities. Choose CPU, memory, battery, current agent activity, or the session timer; select a primary Dynamic Island metric and up to four visible metrics. Start/Stop Live Activity controls monitoring on the iPhone, without changing Mac sleep or cooling settings. Each activity stays attached to its Mac when you switch dashboard devices. Automatic start is available while the iPhone app is open for manual sessions (on by default) and agent work (off by default); dismissal suppresses repeat starts for the same session. CPU and memory require Mac 2.4.2 or later and use read-only host counters, with no privileged helper.

Live Activities receive new snapshots during foreground refreshes and iCloud background notifications. iOS schedules background delivery, so these are periodic readings rather than a per-second system monitor. Data older than two minutes is marked stale, unavailable readings show a dash, and iOS limits an activity to eight hours. Opening it returns to that Mac’s Live Activity screen. Lock Screen and Dynamic Island layouts use the same selected metrics.

...
- iCloud summaries are bounded and private: live counts, safe token/duration deltas, machine state, coarse network availability, alert state, and existing finish actions. They exclude raw skills, file paths, prompts, event details, network names, interfaces, and addresses.
- **Remote Work** is separately opt-in in Mac Settings → Data & iPhone. It adds a bounded per-work-item operational projection. Local IDs are pseudonymized; titles and Codex project labels have their own off-by-default switches. Project labels never fall back to a directory or workspace path. A running Codex task may publish one fixed activity category such as **Edited files** or **Command running**—never the associated command, path, tool payload, output, prompt, or message excerpt.
- Phone-to-Mac context is an explicit, short-lived, 25 MB maximum transfer. The Share Sheet accepts one file or HTTP(S) link; a shared link is stored as a local `.url` receipt and is never fetched by the extension. The Mac validates and copies it into a private `Application Support/Sleep Switch/Remote Inbox` directory. A later **Place…** action requires the Mac owner to choose a folder and makes a non-overwriting local copy; it never chooses a project or feeds an agent automatically.
- The Mac owner can explicitly remove a private Remote Inbox receipt after reviewing or placing it. Removal is confirmed and only deletes that receipt directory; it never reaches into a chosen project folder or alters the original phone-side source.
...
- CloudKit context and result records expire after 24 hours. The reporting Mac removes expired handoffs, offers, and delivery receipts during its regular private-iCloud cleanup (at most every six hours after it is next online). A locally received or explicitly downloaded file remains until its owner removes it; Sleep Switch never silently deletes a result the user chose to keep.
